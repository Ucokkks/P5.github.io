...
if ($result_enum->num_rows > 0) {
    ...
    // Mencari nilai enum 'rumah' di dalam definisi enum
    if (preg_match("/enum\('([^']*)'/", $enum_definition, $matches)) {
        // $matches[1] berisi nilai-nilai enum yang dipisahkan oleh koma
        $enum_values = explode(",", $matches[1]);
        
        // Menampilkan hanya nilai 'rumah'
        if (in_array('rumah', $enum_values)) {
            // Query untuk mengambil data rumah dengan jenis 'rumah'
            $sql_properties = "SELECT nama_rumah, price, location, kamar_tidur, kamar_mandi, gambar FROM rumah WHERE type = 'rumah' ORDER BY RAND() LIMIT 3";
            ...
            if ($result_properties->num_rows > 0) {
                ...
                echo "Tidak ada data rumah dengan jenis 'rumah'.";
            }
        } else {
            echo "Nilai enum 'rumah' tidak ditemukan.";
        }
    } else {
        echo "Enum tidak ditemukan di dalam definisi kolom.";
    }
} else {
